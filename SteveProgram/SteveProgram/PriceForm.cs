...
namespace ButcherBlock
{
    public partial class PriceForm : Form
    {
        ...
        public void loadPrices()
        {
            ...
            sqlite_cmd.CommandText = "INSERT INTO Products (name, price) VALUES ('WholeMuscle Jerkey rounds only', '3.50');";
            sqlite_cmd.ExecuteNonQuery();
            ...
            */
            ...
            while (sqlite_datareader.Read()) // Read() returns true if there is still a result line to read
            {
                ...
                products.Add(new Product(Convert.ToInt32(sqlite_datareader.GetString(0)), sqlite_datareader.GetString(1), Convert.ToDouble(sqlite_datareader.GetString(2))));
        ...
        public void displayProducts()
        {
           ...
           // ListViewItem aFooItem = new ListViewItem("1");
            ...
            {
                ListViewItem aFooItem = new ListViewItem(((Product)products[i]).getId().ToString());
                ListViewItem.ListViewSubItem aSubFooItem1 = new ListViewItem.ListViewSubItem(aFooItem, ((Product)products[i]).getName().ToString());
                ListViewItem.ListViewSubItem aSubFooItem2 = new ListViewItem.ListViewSubItem(aFooItem, ((Product)products[i]).getPrice().ToString()); 
        ...
        private void myListView_Click(object sender, EventArgs e)
        {
            if (myListView.SelectedItems.Count > 0)
            {
                ...
                clickedProductPrice = ((Product)products[myListView.SelectedItems[0].Index]).getPrice();
                new EditForm(clickedProductId, clickedProductName, clickedProductPrice).Show();
                Close();
            }   
        }
